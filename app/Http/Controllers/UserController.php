...
class UserController extends Controller
{
    ...
    public function destroy(User $user)
    {
        ...
        try {
            ...
            // Menangkap error jika user sudah punya riwayat transaksi (Constraint Violation)
            if($e->getCode() == "23000") {
                return redirect()->route('users.index')->with('error', 'GAGAL: Akun ini tidak bisa dihapus karena sudah memiliki riwayat transaksi di gudang (Stok Masuk/Keluar/Mutasi).');
            }
            
            // Untuk error database lainnya
            return redirect()->route('users.index')->with('error', 'Terjadi kesalahan database saat menghapus akun.');
        }
    }
}